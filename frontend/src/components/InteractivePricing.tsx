      ...
      <div className="max-w-[1100px] mx-auto px-4 sm:px-6 lg:px-10">
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-14 items-start">
          ...
          <div className="flex-1 pt-0 lg:pt-1">
            ...
            <div className="flex flex-col bg-[#EDE8E3] border border-[#D8D0C8]">
              {lengths.map((l) => {
                const isActive = active === l.id;
                return (
                  <button
                    key={l.id}
                    onClick={() => setActive(isActive ? null : l.id)}
                    className={`flex items-center justify-between py-3 px-4 text-left transition-all border-b border-[#D0C9C1] last:border-b-0 active:opacity-70 ${
                      isActive ? 'bg-white/80' : 'hover:bg-[#E5DED8]'
                    }`}
                  >
