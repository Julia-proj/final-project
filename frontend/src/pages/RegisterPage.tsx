...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/');
    } catch { /* error in state.auth.error */ }
  };
    ...
    <div className="min-h-screen bg-[#FAF8F6] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-[#666666] text-base font-light">Unete a Keratin Madrid</p>
        </div>
        ...
        <div className="bg-white border border-[#e8e2da] p-8">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div>
              <label htmlFor="password" className="block text-[11px] tracking-[0.2em] uppercase text-[#8B7355] mb-3 font-light">Contrasena</label>
              <input id="password" type="password" name="password" value={form.password}
                onChange={handleChange} required minLength={6} placeholder="Minimo 6 caracteres"
                className="w-full border border-[#e8e2da] px-5 py-3.5 text-base bg-white focus:outline-none focus:border-[#B8A99A]"/>
            </div>
